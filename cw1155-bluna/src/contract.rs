...
use cw1155::{
    ...
    Cw1155BatchReceiveMsg, Cw1155QueryMsg, Cw1155ReceiveMsg, Expiration, IsApprovedForAllResponse,
    TokenId, TokensResponse, TransferEvent,
};
use cw2::set_contract_version;

use crate::error::ContractError;
use crate::msg::{Cw1155ExecuteMsg, InstantiateMsg, TokenInfoResponse};
use crate::state::{APPROVES, BALANCES, MINTER, TOKENS};

// version info for migration info
const CONTRACT_NAME: &str = "crates.io:cw1155-bluna";
...
pub fn execute(
    ...
    match msg {
        ...
        Cw1155ExecuteMsg::Mint {
            ...
            value,
            data,
        } => execute_mint(env, to, token_id, value, data),
...
pub fn execute_mint(
    ...
    if !TOKENS.has(deps.storage, &token_id) {
        ...
        TOKENS.save(deps.storage, &token_id, &total_unbonded_amount)?;
    }
...
pub fn execute_batch_mint(
    ...
    for (token_id, amount) in batch.iter() {
        ...
        if !TOKENS.has(deps.storage, &token_id) {
            // we must save some valid data here
            TOKENS.save(deps.storage, &token_id, &1u64.into())?;
        }
    }
...
pub fn query(deps: Deps, env: Env, msg: Cw1155QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        Cw1155QueryMsg::TokenInfo { token_id } => {
            let total_unbonded_amount = TOKENS.load(deps.storage, &token_id)?;
            to_binary(&TokenInfoResponse { total_unbonded_amount })
        }
